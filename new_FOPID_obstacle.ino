...
void loop(){
  ...
  PCF8574Write(0xC1F| PCF8574Read());   //set Pin High
  value = PCF8574Read() | 0xE0;         //read Pin
  Distance = Distance_test();
  if (Distance < 4)
  { 
    backward(30,30);  //amik lajak
    delay(600);
    stop();
    delay(200);

    forward(30,30);   //corner baring
    delay(10);
    right(40,10);
    delay(600);
    ...
    forward(30,30);    //tekan minyak
    delay(1000);

    left(10,40);       //corner lutut
    delay(600);

    forward(30,30);    //corner atas pasir
    delay(800);
    cooldown = 300;
  }
  else if(Distance >=4)
  ...
  if (sensorValues[1] > 900 && sensorValues[2] > 900 && sensorValues[3] > 900 && cooldown == 0)
  {
...
void forward(int sp_A, int sp_B)
{
...
void backward(int sp_AB1,int sp_AB2)
{
  analogWrite(PWMA,sp_AB1);
  analogWrite(PWMB,sp_AB2);
  digitalWrite(AIN1,HIGH);
  digitalWrite(AIN2,LOW);
  digitalWrite(BIN1,HIGH); 
  digitalWrite(BIN2,LOW);    
...
void right(int sp_A1, int sp_B1)
{
  analogWrite(PWMA,sp_A1);
  analogWrite(PWMB,sp_B1);
  digitalWrite(AIN1,LOW);
  digitalWrite(AIN2,HIGH);
  digitalWrite(BIN1,HIGH); 
  digitalWrite(BIN2,LOW);    
...
void left(int sp_A2, int sp_B2)
{
  analogWrite(PWMA,sp_A2);
  analogWrite(PWMB,sp_B2);
  digitalWrite(AIN1,HIGH);
  digitalWrite(AIN2,LOW);
  digitalWrite(BIN1,LOW); 
  digitalWrite(BIN2,HIGH);    
